...
const caseMaker = (action, target) => {
  ...
  if (action.type === 'set') {
    ...
    result += `      state.${action.target} = action.value;\n      return state;`;
  }
  if (action.type === 'add') {
    //for add we will be pushing the new item into an existing state array
    result += `      state.${action.target}.push(action.item);\n      return state;`;
  }
  if (action.type === 'setIn') {
    ...
    result += `      state.${action.target}[action.${prop}] = action.value;\n      return state;`;
  }
  if (action.type === 'delete') {
    //delete for arrays and objects 
    if (Array.isArray(target)) {
      result += `      state.${action.target}.splice(index, 1);\n      return state;`;
    } else {
      result += `      delete state.${action.target}[key];\n      return state;`;
    }
  }
